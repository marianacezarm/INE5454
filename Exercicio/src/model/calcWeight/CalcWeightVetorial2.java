...
public class CalcWeightVetorial2 extends CalcWeight{
	...
	public void calcWeight(Map<String, Document> documents,
		...
		for (String document : documentName) {
			...
			for (String term : terms) {
				...
				weightQuery = Math.log10(1 + (double)documents.size() / vocabulary.getVocabulary().get(term).getnDoc());
				vocabulary.getVocabulary().get(term).setIdf(weightQuery);
				termsDocument.get(term).setWeight(1 + (double)Math.log10(termDoc.getFrequency()));
			}
		}
